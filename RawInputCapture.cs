...
namespace NvkCommon
{
    ...
    public class RawInputCapture(string Name)
    {
        ...
        public class MouseButtonEventArgs : EventArgs
        {
            ...
            public override string ToString()
            {
                return $"{{Flags={Flags}, Data=0x{Data:X4}}}";
            }
        }
        ...
        private void HandleInput(IntPtr hDevice)
        {
            ...
            var dwSize = 0;
            GetRawInputData(hDevice, RawInputCommand.Input, IntPtr.Zero, ref dwSize, rawInputHeaderSize);
            if (dwSize != GetRawInputData(hDevice, RawInputCommand.Input, out instanceInput, ref dwSize, rawInputHeaderSize))
            {
                Log(LogLevel.Error, $"Failed to get raw input data. outSize={dwSize}, instanceInputSize={rawInputHeaderSize}");
                return;
            }
        ...
        {
            Log(LogLevel.Verbose, $"RegisterRawInputDevices(0x{handle:X8}, ...)");
        ...
        {
            Log(LogLevel.Verbose, $"UnregisterRawInputDevices({handle}, ...)");
            for (int i = 0; i < rawInputDevices.Length; i++)
            {
        ...
        [DllImport("user32.dll")]
        private static extern int GetRawInputData(IntPtr hRawInput, RawInputCommand uiCommand, IntPtr pData, ref int pcbSize, int cbSizeHeader);

        // TODO: Find a way to get this to work...
        [DllImport("user32.dll")]
        private static extern int GetRawInputBuffer([Optional] IntPtr pData, ref int pcbSize, int cbSizeHeader);
